...
void gyro_thread(void const *argument){
	...
	filterState gyroFilterX;
	filterState gyroFilterY;
	filterState gyroFilterZ;
	...
	while (1) {
		...
		switch ( state ){
			...
			case ST_INIT :
				...
				gyro_init();
				initializeFilter(&gyroFilterX, 100);
				initializeFilter(&gyroFilterY, 100);
				initializeFilter(&gyroFilterZ, 100);
			...
			case ST_RUNNING :
				...
				//	offsetGyroData(data);
					...
					//float offset =  computeOffset(data, )
					
					filteredGyroX = modify_filterState(&gyroFilterX, data[0]);
					filteredGyroY = modify_filterState(&gyroFilterY, data[1]-offset);
					filteredGyroZ = modify_filterState(&gyroFilterZ, data[2]);
